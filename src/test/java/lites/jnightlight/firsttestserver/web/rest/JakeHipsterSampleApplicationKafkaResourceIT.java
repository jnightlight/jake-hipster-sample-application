...
import lites.jnightlight.firsttestserver.config.KafkaSseConsumer;
import lites.jnightlight.firsttestserver.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JakeHipsterSampleApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jake-hipster-sample-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jake-hipster-sample-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jake-hipster-sample-application-kafka/unregister"));
                return;
            }
        }
